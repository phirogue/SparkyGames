...
> **Partly superseded 2026-08-03.** Stills are now generated by Claude via
> `tools/genart.py` / the `/genart` skill (`gpt-image-2`), not by hand in
> Kling. The prompts below remain the canonical descriptions of each asset, but
> two ground rules have **reversed** for the API path:
>
> - **Rule 4 is inverted.** Same-room variants ARE made by generating, using
>   `--ref` on the good original (the images *edits* endpoint). `bg_parlor_cold`
>   is an edit of `bg_parlor_warm`, so it is provably the same room. Generating
>   a variant from a fresh description invents new content — that is how a
>   witch appeared in the cold parlor looking nothing like `ref_elspeth`.
> - **Rule 3 is now enforceable.** Reference images really do the consistency
>   work, and `--ref` is how they get attached. Every recurring character needs
>   a `ref_<name>.png` before their second appearance.
>
> Current conventions (framing, prologue canon, folder rules) live in
> [assets/README.md](../../assets/README.md) and CLAUDE.md's art library rules.
