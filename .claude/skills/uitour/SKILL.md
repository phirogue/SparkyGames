...
2. Clear old shots and run the tour (a game window flashes briefly — normal):
   `Remove-Item screenshots/*.png -Force; & "C:\Users\yurim\tools\godot\Godot_v4.4.1-stable_win64_console.exe" --path game -- --tour`
3. Glob `screenshots/*.png` and **Read at minimum**: one `battle_open_*`, one
   ...
   argument names it).
4. Judge each against `assets/incoming/ui_objective.png` and the checklist:
...
5. Report defects found (file:line of the responsible code where possible),
